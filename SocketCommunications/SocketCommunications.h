...
#define ResizeSocketBuffers
	...
	bool sendTo(const SenderSocketInfo& socketInfo, const std::vector<char>& data);
	int sendToWithSelect(const SenderSocketInfo& socketInfo, const std::vector<char>& data, float timeoutSeconds = 1e-4f);

	// output buffer may be resized to only hold valid bytes
	bool receiveFrom(const SenderSocketInfo& socketInfo, std::vector<char>& buffer, sockaddr_in& outSenderAddr);
	bool receiveFromSelect(const SenderSocketInfo& socketInfo, std::vector<char>& buffer, sockaddr_in& senderaddr, float timeout =  1e-4f);
