...
namespace invisCom {
	...
	bool sendTo(const SocketInfo& socketInfo, const std::vector<char>& buffer)
	{
		bool ret = false;
	...
	int sendToWithSelect(const SocketInfo& socketInfo, const std::vector<char>& data, float timeout)
	{
		int res = 0;
		int retry = 10;
	...
	bool receiveFrom(const SocketInfo& socketInfo, std::vector<char>& buffer, sockaddr_in& senderaddr)
	{
		bool ret = false;
	...
	bool receiveFromSelect(const SocketInfo& socketInfo, std::vector<char>& buffer, sockaddr_in& senderaddr, float timeout)
	{
		bool ret = false;
